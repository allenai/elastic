...
class IDAUp(nn.Module):
    ...
    def forward(self, layers):
        ...
        for i in range(1, len(layers)):
            ...
            x = node(torch.cat([x, layers[i][:, :, :x.size(2), :x.size(3)]], 1))
            y.append(x)
        return x, y
...
class DLASeg(nn.Module):
    def __init__(self, base_name, classes, down_ratio=2):
        ...
        self.base = dla.__dict__[base_name](return_levels=True, seg=True)
        channels = self.base.channels
        # print(channels, self.first_level)
...
def dla60x_seg(classes, **kwargs):
    model = DLASeg('dla60x', classes, **kwargs)
    return model


def dla60x_elastic_seg(classes, **kwargs):
    model = DLASeg('dla60x_elastic', classes, **kwargs)
    return model
